...
# Скрипт для тестирования /api/analyze в режиме SAM3 через curl
# Использование: ./test_curl_sam3.sh [image_path] [railway_url]
...
echo "=" | tr -d '\n' | head -c 80 && echo ""
echo "ТЕСТИРОВАНИЕ /api/analyze (SAM3 режим) через curl"
echo "=" | tr -d '\n' | head -c 80 && echo ""
echo ""
...
# Формируем JSON payload для SAM3
PAYLOAD=$(cat <<EOF
{
